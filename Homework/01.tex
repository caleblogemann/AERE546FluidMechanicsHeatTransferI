  ...
  \item[\#1] % Done
    \item[(a)] % Done
      ...
      \[
        ...
        \frac{1}{6} f'''(x_0) (x - x_0)^3 + O((x - x_0)^4)
      \]
      Note that this polynomial has errors of order $(x - x_0)^4$, so the
    ...
    \item[(b)] % Done
      ...
      \begin{gather*}
        f_{-1} = a - b \Delta x + c (\Delta x)^2 \\
        f_0 = a \\
        f_1 = a + b \Delta x + c (\Delta x)^2
      \end{gather*}
      Clearly $a = f_0$.
      The first and third equations can be summed to find $c$.
      \begin{gather*}
        f_{-1} + f_1 = 2f_0 + 2 c (\Delta x)^2 \\
        c = \frac{f_{-1} - f_0 + f_1}{2 (\Delta x)^2}
      \end{gather*}
